...
test.describe("EmailSubscribeForm - main page", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto("/en");
    // Scroll to the stay-in-touch section
    await page.locator("#stay-in-touch").scrollIntoViewIfNeeded();
  });

  test("renders email input and submit button", async ({ page }) => {
    const section = page.locator("#stay-in-touch");
  ...
  test("submits valid email and shows success message", async ({ page }) => {
    ...
    await expect(section.locator("text=Thanks!")).toBeVisible({ timeout: 5000 });
  });
  ...
  test("shows error message on API failure", async ({ page }) => {
    ...
    await expect(section.locator("text=Something went wrong")).toBeVisible({ timeout: 5000 });
  });
});
