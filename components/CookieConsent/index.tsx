...
// The banner cannot be dismissed without an explicit choice (GDPR/LGPD), so
// closeBanner is never wired up, and the native dialog's "close" event
// (which only fires from our own imperative .close() below) has nothing to
// report back to.
const noop = () => {};

export default function CookieConsent() {
  const t = useTranslations("cookieConsent");
  const { showBanner, preferences, acceptAll, rejectAll, saveCustomPreferences } =
    useCookieConsent();
  ...
  const dialogRef = useDialogElement(showBanner, noop);
  ...
  return (
    <dialog
      ...
      className="m-auto max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white p-6 shadow-xl dark:bg-gray-800 sm:p-8 backdrop:bg-black/50"
    >
      {!showCustomize ? (
        // Main banner view
        <>
